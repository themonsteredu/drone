...
## 운항 점검과 활동 기록 (2026-09-06)

- 항로를 고르고 선택 이유를 한 줄 적은 뒤 출발 전 세 항목을 직접 체크해야 출발할 수 있습니다. 항로 변경 시 이유와 확인 항목을 비워 다시 판단하게 합니다. 출발 후에는 원래 판단과 점검 내용을 변경할 수 없습니다.
- 의료 인계는 정상 착륙 후 착륙장 안에서 기체가 멈추고 모터 회전도 끝난 상태에만 인정합니다. 재시동·재이륙·착륙장 이탈 시 인계 대기를 해제하며, 다시 착륙해야 인계할 수 있습니다. 제한 시간과 같은 순간 또는 이후의 인계는 성공으로 계산하지 않습니다.
- 연결이 끊겨 대기하는 동안 누른 임무 버튼은 재연결 뒤 자동 실행하지 않습니다.
- 임무 종료 결과에서 이름 또는 모둠, 다음 운항에서 바꿔볼 점을 작성해 **기록 파일 저장** 또는 **파일 공유**를 누를 수 있습니다. 항로·선택 이유·출발 점검·평가 항목·충돌·항로 이탈·배터리·화물 인계·학생 소감을 독립 HTML 파일에 담습니다. 성공과 시간 초과 모두 기록할 수 있습니다.
- 기록 파일은 브라우저에서 다시 열거나 인쇄해 사용하고 수업 제출함에 직접 첨부할 수 있습니다. 공유를 지원하지 않는 기기는 파일 다운로드로 안내합니다. 학생 입력은 HTML로 실행되지 않도록 이스케이프하고, 파일은 외부 통신·스크립트를 사용하지 않습니다.
- **현재 저장 방식은 파일 내보내기입니다.** 학생 계정, 온라인 기록 보관함, 교사 제출함 및 Career Log 자동 전송은 연결하지 않았습니다. 앱을 나가기 전에 파일을 저장해야 하며, 다운로드 요청을 실제 저장·제출 완료로 표시하지 않습니다. 실물 USB 조종기·3D 화면과 각 기기의 다운로드·공유 동작은 수업 기기에서 최종 확인해야 합니다.

## 3D 디자인 업데이트 (2026-09-06)

- 체험 범위는 무인 드론으로 의약품과 현장 정보를 운송·전달하는 항공모빌리티 운항입니다. 운항 요청 → 항로·기상 판단 → 조종 → 정밀 착륙 → 인계·복귀의 기존 수업 흐름을 유지합니다.
- 비행 소리는 기본 꺼짐이며 사용자가 버튼으로 켠 뒤 재생됩니다. 모터 회전·이동 속도·강풍 구역에 따라 로컬 Web Audio 합성 음향이 달라지고, 지면 접촉에만 짧은 착륙음이 발생합니다. 실제 기체 녹음은 아니며 탭을 숨기거나 체험 화면을 나가면 재생을 중지합니다.
- 다음 지점의 기체 기준 방향과 수평거리, 현재 행동을 한곳에 표시합니다. 의료 배송은 선택 항로의 경유점을 따르고, 수색 완료 시 안내가 지휘소 복귀 착륙장으로 바뀝니다. 방향 화살표는 위치 안내이며 장애물 회피나 자동 조종 명령이 아닙니다.
- 실제 지면 접촉에서만 작은 기체 안정화 움직임을 표시하며, 동작 줄이기 설정에서는 생략합니다. 조종 감도·착륙 판정·점수는 변경하지 않습니다.
- 실제 비행 상태와 연결된 산업용 기체 외형: 카본 암, 교체형 배터리, 착륙 다리, 카메라와 의료 운송함.
- 아스팔트·암석·잔디의 1K 표면 재질을 프로젝트에 포함합니다. 이 재질은 AI로 제작한 합성 이미지이며 실제 장소를 촬영한 사진/스캔이 아닙니다. 런타임 외부 이미지 다운로드나 생성 API 비용은 없습니다.
- 월드 좌표 기반 재질 배치, 자연광 환경 반사, 세분화한 암석·능선과 잎 인스턴싱을 적용합니다. 학교 노트북의 기존 30fps/해상도 제한은 유지합니다.
- 운항 화면을 넓히고 임무 안내를 짧게 표시합니다. 임무 중 고도·속도·시간·배터리는 하단 계기 한 곳에만 표시합니다.
- WebGL을 지원하지 않는 환경에서는 기존 Canvas 호환 화면으로 전환됩니다.
- 이번 작업의 검증용 브라우저는 WebGL context 생성이 비활성화되어 있어 새로운 3D 외형과 실제 USB 조종기 실기 검증은 완료하지 못했습니다. 빌드·타입 검사와 조종/임무 테스트는 별도로 확인합니다.

USB 조종기의 실제 입력을 웹브라우저에서 안정적으로 읽고, 검증된 `Common ControllerState`로 조종법 안내부터 기초 훈련, 자격시험, 항공모빌리티 임무까지 진행하는 교육용 시뮬레이터입니다. 기존 Web Serial, RAW monitor, BYROBOT parser, CRC, 0x71 Joystick, 0x70 Button 진단 기능은 그대로 유지합니다.
...
검증된 0x71 stick 순서 `[Left X, Left Y, Right X, Right Y]`에는 다음 기본 의미 배치를 적용합니다. 현재 교실 조종기에서 확인된 수평축 극성에 따라 **Left X(Yaw)와 Right X(Roll)만 ControllerProfile에서 반전**합니다.

```text
Left Y  → Throttle → 상승/하강
Left X  → Yaw      → 좌우 회전
Right Y → Pitch    → 전진/후진
Right X → Roll     → 기체 기준 좌우 이동
```

왼쪽 스틱 오른쪽은 기체의 오른쪽/시계 방향 Yaw, 오른쪽 스틱 오른쪽은 기체 기준 오른쪽 이동이어야 합니다. 빨간 선은 기체의 앞방향입니다.

Left X와 Right X의 보정은 실제 조종기의 물리적 오른쪽 입력이 각각 semantic Yaw `+`, Roll `+`가 되도록 ControllerProfile 경계에서만 적용합니다. Mode 2 시동의 물리적 5시·7시 조합은 보정 후 `Throttle - / Yaw + / Pitch - / Roll -`가 됩니다. 네 축의 비행 방향과 시동 도달 가능성은 `tests/axis-preset-contract.test.mjs`가 함께 검증합니다.

학생용 **Mode 2 기본 조종**은 스틱 시동과 실제 스로틀 이륙이 기본입니다. 다만 연결 점검, 접근성 보조와 수업 복구를 위해 화면 아래의 **시동 / 이륙 / 착륙** 버튼도 유지합니다. 화면 시동은 동일한 `ARMED` 상태를 만들고, 화면 이륙은 `TAKEOFF → FLIGHT` 안전 상승을 실행하므로 튜토리얼과 자격시험 상태 기록에서도 빠지지 않습니다.

1. 양쪽 스틱을 아래쪽 바깥으로 벌립니다.
   - 왼쪽 스틱: 아래+왼쪽, 약 7시 방향
   - 오른쪽 스틱: 아래+오른쪽, 약 5시 방향
2. 아래 방향은 `0.40`, 바깥 방향은 `0.25` 임계값을 끊지 않고 `2초` 유지합니다. 임계값을 벗어나도 `0.65초` 안에 돌아오면 유지 시간이 이어지고, 그보다 길게 놓으면 처음부터 다시 시작합니다. 시동 완료 순간에는 반드시 임계값 안에 있어야 하므로 입력이 끊긴 채로는 시동이 걸리지 않습니다.
...
조종기 연결이 끊기거나 입력 준비 조건을 잃거나 최신 stick 입력이 1200ms 이상 멈추면 잔류 속도·회전 입력을 즉시 중립화합니다. 탭이 백그라운드로 가도 motion을 중립화하고 복귀 시 큰 frame delta를 적용하지 않습니다.
...
| 조종 방식 | Mode 2 기본 조종 | Mode 2 기본 조종 / 사용자 설정 |
| 헤드리스 모드 | 끔 | 끔 / 켬 |
| 자세 안정화 | 켬 | 켬 / 끔 |
...
3D 화면은 기체 뒤에서 앞을 바라봅니다. 기체 기수가 로컬 `+Z`이고 오른손 좌표계이므로 **월드 `+X`가 화면 왼쪽**에 옵니다. 내부 yaw는 `+Z`에서 `+X`로 갈 때 증가하도록 정의되어 있어서, 학생이 "오른쪽"이라고 부르는 회전은 내부 yaw를 **감소**시킵니다.

그래서 학생에게 보이는 값이나 학생용 좌우 지시를 판정하는 곳은 반드시 부호를 한 번 뒤집습니다.

| 위치 | 처리 |
| --- | --- |
| 화면 `방향` 표시 | `studentHeadingRadians()` (`simulator/flight-model.ts`) |
| 조종법 `오른쪽 회전` 판정 | `tutorial-runtime.ts`의 `yaw_delta` 분기 |

ControllerProfile이 두 수평축을 반전하는 것도 같은 이유입니다. 반전 덕분에 화면 동작은 조종기와 일치하고, 시동 조합은 물리적으로 **벌리기**가 됩니다. 이 세 가지(축 반전, 화면 부호, 시동 방향)는 서로 맞물려 있어서 하나만 바꾸면 나머지가 깨집니다.
...
드론과 임무 환경은 Three.js 기반의 실시간 3D 외형과 프로젝트에 포함된 재질로 렌더링합니다. WebGL이 비활성화된 기기에는 Canvas 호환 화면을 사용합니다. 충돌과 경로 판정은 데이터 기반 도메인 로직에 있고, 시각 요소는 `DroneVisual`/`DroneScenePresentation`으로 분리되어 있어 향후 GLB/GLTF와 미래도시 외형으로 교체할 수 있습니다.
...
프로덕션 주소는 `https://drone-six-smoky.vercel.app` 입니다.

**지금 떠 있는 화면이 어느 빌드인지 먼저 확인하고 실기 테스트를 시작하세요.** 축 부호처럼 실기로만 확인되는 수정은, 배포가 안 된 상태에서 테스트하면 고치기 전과 똑같은 증상이 그대로 나옵니다. 실제로 그렇게 한 사이클을 통째로 날린 적이 있습니다.

조종기 없이 판별하는 방법:

| 확인 위치 | 최신 빌드 | 옛 빌드 |
| --- | --- | --- |
| 비행 화면 아래 | 접힌 **수업 준비** 있음 | 비행 설정이 펼쳐진 채 |
| 비행 상태 표시 | 학생 조종석 `현재 상태` 한 곳 | 3곳에 중복 |
| 긴급 안전 착륙 버튼 | 비행 화면 위 1개 | 화면 위와 버튼 줄에 2개 |

조종기로 판별하는 방법:

| 조작 | 최신 빌드 | 옛 빌드 |
| --- | --- | --- |
| Mode 2 시동 | 양쪽 스틱을 **벌려서** 2초 | 모아야 걸림 |
| 왼쪽 스틱 오른쪽 | `방향` 값 증가 | 값 감소 |
| 오른쪽 스틱 오른쪽 | 기체가 오른쪽으로 | 왼쪽으로 |
...
엄격하게 해석된 BYROBOT 0x71 8-byte profile에는 기본 배치 `Left X→Yaw`, `Left Y→Throttle`, `Right X→Roll`, `Right Y→Pitch`를 적용합니다. 실기 확인 결과에 따라 **네 축 모두 입력 부호를 반전하지 않습니다**. 이 preset은 codec ID, DataType `0x71`, payload 길이 `8`, axis 수 `4`가 모두 맞을 때만 활성화됩니다.
...
접힌 **개발자 정보 보기** 안의 **조종기 기본 조작 확인**은 새로운 제품 프로필을 만들 때 사용하는 기록 도구입니다.
...
- 산업용 드론, 재질이 적용된 지형·건물, 자연광 기반 Three.js 환경과 Canvas 호환 화면
...
실제 조종기 세션에서 다음 세 증상을 전달받았고, 모두 축 부호 하나씩이 원인이었습니다.

| 증상 | 원인 |
| --- | --- |
| 양쪽 스틱을 모아도 Mode 2 시동이 걸리지 않음 | Yaw 보정과 시동 Roll 조건의 불일치 |
| 오른쪽 Yaw에서 화면 `방향` 값이 음수로 감소 | Left X 보정 누락 |
| raw 값은 정상인데 Roll이 조종기와 반대로 이동 | Right X 보정 누락 |

Yaw 방향과 시동은 같은 Left X 보정을 공유합니다. 또한 오른쪽 스틱 7시 방향은 Roll 비행 방향 보정을 거친 뒤 semantic Roll 음수가 되므로, 시동 검출기도 같은 부호를 사용해야 합니다. 한쪽만 바꾸면 비행 방향이나 시동 중 하나가 다시 깨지므로 `tests/axis-preset-contract.test.mjs`가 실제 물리 코너부터 두 조건을 함께 검사합니다.
...
1. **기본값 복원**으로 일반 65%, Mode 2 기본 조종, 헤드리스 끔, 자세 안정화 켬, Dead Zone 0.10, 감도 100%, Expo 0.45를 확인합니다.
2. **조종법 익히기 시작**을 누르고 양쪽 스틱을 아래 바깥으로 벌립니다(왼쪽 7시, 오른쪽 5시). 임계값을 중간에 벗어나지 않고 2초 유지해 `ARMING`의 진행도가 차고 `ARMED(시동 완료)`가 되는지 확인합니다. 0.65초를 넘겨 스틱을 놓으면 시간이 초기화되어야 합니다.
...
4. 빨간 선을 기체의 앞으로 보고 네 축을 하나씩 확인합니다. 왼쪽 위/아래는 상승/하강, 왼쪽 오른쪽/왼쪽은 오른쪽/왼쪽 Yaw, 오른쪽 위/아래는 전진/후진, 오른쪽 오른쪽/왼쪽은 기체 기준 오른쪽/왼쪽 이동이어야 합니다. 구현에서는 수평축 보정을 ControllerProfile에만 적용하고 비행 물리는 공통 의미축을 그대로 사용합니다.
...
4. 연결을 해제하거나 1200ms 이상 입력이 끊겼을 때 드론의 수평 속도와 Yaw rate가 중립화되는지 확인합니다.
...
- Throttle/Yaw/Pitch/Roll 네 방향이 모두 학생이 보는 스틱 조작과 일치
