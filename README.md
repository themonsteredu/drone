...
검증된 0x71 stick 순서 `[Left X, Left Y, Right X, Right Y]`에는 다음 기본 의미 배치를 적용합니다. **네 축 모두 부호를 반전하지 않고 그대로** 사용합니다.
...
예전에는 Left X(Yaw)와 Right X(Roll)를 반전했으나 실기에서 둘 다 반대로 동작해 되돌렸습니다. 두 X 축은 같은 종류의 하드웨어이므로 좌우 극성이 서로 달라지면 둘 중 하나가 틀린 것입니다. Left X 부호는 Mode 2 시동도 함께 결정합니다. 시동 동작은 semantic Yaw가 `+임계값` 이상이어야 하므로, Left X가 반전되어 있으면 임계값을 아무리 낮춰도 스틱으로는 시동을 걸 수 없습니다. 네 축의 방향, 두 스틱의 극성 일치, 시동 도달 가능성은 `tests/axis-preset-contract.test.mjs`가 함께 검증합니다.
...
| 증상 | 원인 |
| --- | --- |
| 양쪽 스틱을 모아도 Mode 2 시동이 걸리지 않음 | Left X 반전 |
| 오른쪽 Yaw에서 화면 `방향` 값이 음수로 감소 | Left X 반전 |
| raw 값은 정상인데 Roll이 조종기와 반대로 이동 | Right X 반전 |

Left X 반전은 앞의 두 증상을 한꺼번에 만들었습니다. 스틱 오른쪽이 semantic Yaw 음수가 되는데 시동 동작은 semantic Yaw `+임계값` 이상을 요구하므로, 스틱으로는 도달할 수 없는 상태였습니다. **임계값을 낮추거나 입력 신선도 창을 늘리는 것으로는 해결되지 않는 종류의 문제**였습니다. 실제로 그 두 가지를 시도한 변경이 있었지만 증상이 그대로였던 이유가 이것입니다.

두 반전을 모두 되돌려 지금은 네 축이 전부 직결입니다. 이 상태에서 두 X 축의 좌우 극성이 처음으로 일치합니다. 극성이 서로 어긋나 있으면 둘 중 하나가 틀린 것이므로, `tests/axis-preset-contract.test.mjs`가 그 일치까지 함께 검사합니다.
...
4. 빨간 선을 기체의 앞으로 보고 네 축을 하나씩 확인합니다. 왼쪽 위/아래는 상승/하강, 왼쪽 오른쪽/왼쪽은 오른쪽/왼쪽 Yaw, 오른쪽 위/아래는 전진/후진, 오른쪽 오른쪽/왼쪽은 기체 기준 오른쪽/왼쪽 이동이어야 합니다. 구현에서는 네 축 모두 입력 부호를 그대로 사용합니다.
