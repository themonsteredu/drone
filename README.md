...
검증된 0x71 stick 순서 `[Left X, Left Y, Right X, Right Y]`에는 다음 기본 의미 배치를 적용합니다. **Right X(Roll) 한 축만 반전**하고 Throttle, Yaw, Pitch 방향은 유지합니다.

```text
Left Y  → Throttle → 상승/하강
Left X  → Yaw      → 좌우 회전 (부호 유지)
Right Y → Pitch    → 전진/후진
...
Left X(Yaw)를 한때 반전한 적이 있으나 실기에서 오른쪽 조작이 반시계 회전으로 나와 되돌렸습니다. 이 부호는 Mode 2 시동도 함께 결정합니다. 시동 동작은 semantic Yaw가 `+임계값` 이상이어야 하므로, Left X가 반전되어 있으면 임계값을 아무리 낮춰도 스틱으로는 시동을 걸 수 없습니다. 네 축의 방향과 시동 도달 가능성은 `tests/axis-preset-contract.test.mjs`가 함께 검증합니다.
...
실제 조종기 세션에서 다음 두 증상을 전달받았습니다.

- 양쪽 스틱을 모아도 Mode 2 시동이 걸리지 않음
- 오른쪽으로 Yaw를 하면 화면 `방향` 값이 음수로 감소

두 증상의 원인은 하나입니다. Left X(Yaw)가 반전되어 있어 스틱 오른쪽이 semantic Yaw 음수가 되었고, 시동 동작은 semantic Yaw `+임계값` 이상을 요구하므로 스틱으로는 도달할 수 없는 상태였습니다. 임계값을 낮추거나 입력 신선도를 늘리는 것으로는 해결되지 않는 종류의 문제였습니다. Left X 반전을 되돌려 두 증상을 함께 해소했습니다.

아직 확인이 남은 것: **Right X(Roll) 부호**. Left X와 Right X는 같은 종류의 축인데 현재 Right X만 반전되어 있어 좌우 극성이 서로 다릅니다. Roll이 반대로 움직인다는 보고는 아직 없으므로 임의로 바꾸지 않았고, 조종법 5단계(오른쪽 Roll 이동)에서 실기로 확인해야 합니다.
...
4. 빨간 선을 기체의 앞으로 보고 네 축을 하나씩 확인합니다. 왼쪽 위/아래는 상승/하강, 왼쪽 오른쪽/왼쪽은 오른쪽/왼쪽 Yaw, 오른쪽 위/아래는 전진/후진, 오른쪽 오른쪽/왼쪽은 기체 기준 오른쪽/왼쪽 이동이어야 합니다. 구현에서는 Roll 입력 부호만 보정했고 Throttle, Yaw, Pitch 부호는 변경하지 않았습니다.
...
- Throttle/Pitch/Yaw 방향은 기존과 같고 Roll은 학생이 보는 오른쪽/왼쪽 조작과 일치
